    ...
    <div class="card p-4 shadow-sm">
        ...
        <form id="question-form" method="POST" action="{{ route('questions.update', $question->id) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="question" id="question" value="{{ old('question', $question->question) }}" class="form-control" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="type" id="type" class="form-select" required>
                    ...
                    <option value="multiselect" {{ (old('type', $question->type) === 'multiselect') ? 'selected' : '' }}>Multi-Select</option>
                    <option value="object" {{ (old('type', $question->type) === 'object') ? 'selected' : '' }}>Object-Oriented (4 Sub-Questions)</option>
                </select>
            </div>
            ...
            <div id="option-fields" style="display:none;">
                ...
                <div id="options-container">
                    @php
                        $oldOptions = old('options', is_array($question->options) ? $question->options : ['']);
                    @endphp
            ...
            <div id="sub-question-fields" style="display:none;">
                <label class="form-label">Sub Questions <small>(exactly 4)</small></label>
                @php
                    $oldSubs = old('sub_questions', $question->sub_questions ?? ['', '', '', '']);
                @endphp
                @for ($i = 0; $i < 4; $i++)
